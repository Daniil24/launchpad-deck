...
- 🎛 **Макро-дека** (як Stream Deck) — призначай на кнопки запуск програм, медіа та гучність, мут мікрофона (працює в Discord), блокування ПК, гарячі клавіші, керування **OBS** і багато іншого.
- 🎆 **Світломузика** — пед реагує на звук із ПК: бас б'є, хай-хети іскрять, дропи спалахують. **60+** сцен з анімаціями та персонажами.
...
- 🎥 **OBS Studio** — зміна сцени, старт/стоп запису, ефір, пауза, мут джерела, повтор, вірт-камера (через obs-websocket).
...
- 🚀 **Автозапуск** з Windows — сам знаходить пед і піднімає останній конфіг.
- 💾 Експорт/імпорт розкладки, вбудоване **навчання** (17 кроків), плавні анімації запуску та закриття.
...
- **60+ генеративних режимів**: спектр, барабани, хай-хети, персонажі (🐍 змійка, 🕺 танцюрист, 👾 прибулець, 🤖 робот), феєрверки, калейдоскоп, плазма, тунелі та ін.
...
- Чутливість і яскравість регулюються **просто з педа** (правий стовпець / верхній ряд).
...
| 🎥 OBS-студія | керування OBS | `scene:Гра` · `record` · `stream` · `mute:Мікрофон` · `replay` · `virtualcam` |
...
<details>
<summary><b>Покроково — підключити OBS до деки</b></summary>

1. У **OBS Studio** відкрий **Інструменти → налаштування obs-websocket** (WebSocket Server Settings).
2. Увімкни **Enable WebSocket server**. Порт за замовчуванням — `4455`.
3. Якщо стоїть галочка **Enable Authentication** — скопіюй пароль (**Show Connect Info**).
4. У **Launchpad Deck** → картка **Ще → OBS** — встав цей пароль і збережи.
5. Признач педам дії типу **OBS-студія**:
   - зміна сцени — `scene:ТочнаНазваСцени`
   - запис — `record`, ефір — `stream`, пауза — `pause`
   - мут джерела — `mute:ТочнаНазваДжерела`
   - повтор — `replay`, вірт-камера — `virtualcam`

> ⚠️ Назви сцен і джерел мають **точно** збігатися з тим, що в OBS (з урахуванням регістру та пробілів).
...
- **Автозапуск** — увімкни галочку, і дека стартує разом з Windows, сама знаходить пед і піднімає останній профіль.
- **Навчання** — вбудований гід із 17 кроків проведе по всіх функціях.
